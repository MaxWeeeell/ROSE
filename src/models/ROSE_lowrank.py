...
class ROSE(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int,n_embedding:int, num_patch:int, mask_mode:str = 'patch',mask_nums:int = 3,
                 n_layers:int=3, d_model=128, n_heads=16, shared_embedding=True, d_ff:int=256,num_token=1,
        ...
        # Backbone
        self.backbone = ROSEEncoder(c_in, num_patch=num_patch+num_token, patch_len=patch_len, 
        ...
        if head_type == "pretrain":
            self.head_reconstruct = PretrainHead(d_model, patch_len, head_dropout) 
